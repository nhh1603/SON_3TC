...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("library_path1", "/libraries/delays.lib");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = FAUSTFLOAT((fVec0[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp6))) & 1023] * (fTemp7 + fTemp5) + (fRec0[0] - fTemp7) * fVec0[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp6 + 1))) & 1023]) * fTemp0 + (fVec0[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp3))) & 1023] * (fTemp4 + fTemp5 - fSlow1) + (fSlow1 + (fRec0[0] - fTemp4)) * fVec0[(IOTA0 - std::min<int>(1001, std::max<int>(0, iTemp3 + 1))) & 1023]) * (1.0f - fTemp0));
			fRec0[1] = fRec0[0];
			IOTA0 = IOTA0 + 1;
		}
	}
...
pitchShifter::pitchShifter() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void pitchShifter::updateImp(void)
{
#if MIDICTRL
...
void pitchShifter::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void pitchShifter::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float pitchShifter::getParamValue(const std::string& path)
{
