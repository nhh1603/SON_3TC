...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "testFaust.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("testFaust");
		ui_interface->addHorizontalSlider("feedback", &fHslider4, FAUSTFLOAT(0.75f), FAUSTFLOAT(0.0f), FAUSTFLOAT(0.99f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("freq", &fHslider2, FAUSTFLOAT(5e+01f), FAUSTFLOAT(3e+01f), FAUSTFLOAT(8e+03f), FAUSTFLOAT(1.0f));
		ui_interface->addHorizontalSlider("gain", &fHslider3, FAUSTFLOAT(0.1f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("lfreq", &fHslider0, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.01f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("lrange", &fHslider1, FAUSTFLOAT(3e+02f), FAUSTFLOAT(1e+01f), FAUSTFLOAT(3e+03f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow2 = std::max<float>(1.1920929e-07f, std::fabs(float(fHslider2)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp0 = std::tan(fConst2 * (fSlow1 * (2.0f * (1.0f - std::fabs(2.0f * fRec1[0] + -1.0f)) + -1.0f + 1.0f) + 5e+01f));
			float fTemp1 = 1.0f / fTemp0;
			float fTemp2 = (fTemp1 + 0.2f) / fTemp0 + 1.0f;
			float fTemp3 = fSlow3 + fRec3[1] + -1.0f;
			int iTemp4 = fTemp3 < 0.0f;
			float fTemp5 = fSlow3 + fRec3[1];
			fRec3[0] = ((iTemp4) ? fTemp5 : fTemp3);
			float fThen1 = fSlow3 + fRec3[1] + fSlow4 * fTemp3;
			float fRec4 = ((iTemp4) ? fTemp5 : fThen1);
			fRec2[0] = fSlow5 * (2.0f * fRec4 + -1.0f) - (fRec2[2] * ((fTemp1 + -0.2f) / fTemp0 + 1.0f) + 2.0f * fRec2[1] * (1.0f - 1.0f / mydsp_faustpower2_f(fTemp0))) / fTemp2;
			fRec0[IOTA0 & 16383] = fSlow6 * fRec0[(IOTA0 - 10001) & 16383] + (fRec2[2] + fRec2[0] + 2.0f * fRec2[1]) / fTemp2;
			output0[i0] = FAUSTFLOAT(fRec0[IOTA0 & 16383]);
			fRec1[1] = fRec1[0];
			fRec3[1] = fRec3[0];
			fRec2[2] = fRec2[1];
			fRec2[1] = fRec2[0];
	...
	#define FAUST_FILE_NAME "testFaust.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("feedback", fHslider4, 0.75f, 0.0f, 0.99f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider2, 5e+01f, 3e+01f, 8e+03f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("gain", fHslider3, 0.1f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("lfreq", fHslider0, 0.5f, 0.01f, 5e+01f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("lrange", fHslider1, 3e+02f, 1e+01f, 3e+03f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, feedback, "feedback", fHslider4, 0.75f, 0.0f, 0.99f, 0.01f) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider2, 5e+01f, 3e+01f, 8e+03f, 1.0f) \
		p(HORIZONTALSLIDER, gain, "gain", fHslider3, 0.1f, 0.0f, 1.0f, 0.01f) \
		p(HORIZONTALSLIDER, lfreq, "lfreq", fHslider0, 0.5f, 0.01f, 5e+01f, 0.01f) \
		p(HORIZONTALSLIDER, lrange, "lrange", fHslider1, 3e+02f, 1e+01f, 3e+03f, 1.0f) \
...
testFaust::testFaust() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void testFaust::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void testFaust::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float testFaust::getParamValue(const std::string& path)
{
